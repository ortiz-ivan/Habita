...
export default function HabitacionesPage() {
  ...
  const [viewTarget, setViewTarget]     = useState<Habitacion | null>(null)
  ...
  const openEdit   = (h: Habitacion) => { setEditTarget(h); setApiError(''); setViewTarget(null); setModalOpen(true) }
  ...
  return (
    <div>
      ...
      <div className="rounded mb-8 bg-surface-1 border border-border">
        <div className="flex flex-wrap items-center gap-3 px-6 py-4">
          <div className="relative flex-1 min-w-[350px] max-w-xl">
            ...
            <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por número o descripción..." className={`${inpFilter} pr-3 w-full`} style={{ paddingLeft: '2.25rem' }} />
          </div>

          <SelectInput value={piso} onChange={(e) => setPiso(e.target.value)} className={`${inpFilter} px-3`}>
          ...
          {hayFiltros && (
            <button onClick={() => { setSearch(''); setEstado(''); setPiso('') }} className="flex items-center gap-1.5 text-[13px] font-medium cursor-pointer transition-colors shrink-0" style={{ color: 'var(--color-stone-text)' }} onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--color-brand)' }} onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--color-stone-text)' }}>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.2} stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21" /></svg>}
          ...
          action={!hayFiltros && <Button onClick={openCreate} className="px-5">+ Nueva habitación</Button>}
        />
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5">
            {data.results.map((h, i) => (
              <div key={h.id} style={{ animation: 'slide-up-fade 0.38s ease both', animationDelay: `${Math.min(i * 0.045, 0.36)}s` }}>
      ...
      <Modal isOpen={!!viewTarget} onClose={() => setViewTarget(null)} title={`Habitación ${viewTarget?.numero}`} size="lg">
        {viewTarget && <HabitacionDetail h={viewTarget} onEdit={() => openEdit(viewTarget)} onDelete={() => setDeleteTarget(viewTarget)} />}
      </Modal>
