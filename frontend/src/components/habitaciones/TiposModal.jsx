...
export function TiposModal({ isOpen, onClose }) {
  ...
  const handleSubmit = (data, applyToAll) => {
    setApiError('')
    if (editingTipo) {
  ...
  const openEdit   = (t) => { setEditingTipo(t);   setApiError(''); setView('form') }
...
function TipoRow({ tipo, onEdit, onDelete }) {
