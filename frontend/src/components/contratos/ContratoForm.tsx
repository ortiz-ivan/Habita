...
interface ContratoFormProps {
  defaultValues?: Partial<ContratoRead>
  onSubmit: (data: ContratoWrite, cuotasGarantia: number) => void
  onCancel?: () => void
  isLoading?: boolean
  apiError?: string
}

export default function ContratoForm({ defaultValues, onSubmit, onCancel, isLoading, apiError }: ContratoFormProps) {
  const [submodal, setSubmodal]             = useState<'habitacion' | 'inquilino' | null>(null)
  ...
  const { register, handleSubmit, setValue, watch, control, formState: { errors, dirtyFields } } = useForm<FormValues>({
    ...
    defaultValues: defaultValues
      ? {
          ...defaultValues,
          habitacion: typeof defaultValues.habitacion === 'object' ? defaultValues.habitacion?.id : defaultValues.habitacion,
          inquilino:  typeof defaultValues.inquilino  === 'object' ? defaultValues.inquilino?.id  : defaultValues.inquilino,
        } as FormValues
      : (() => {
          ...
          return { estado: 'activo' as const, deposito: 0, observacion: '', fecha_inicio: fmt(today), fecha_fin: fmt(fin) }
        })(),
  })
  ...
  const currentHabId  = typeof defaultValues?.habitacion === 'object' ? defaultValues.habitacion?.id : defaultValues?.habitacion
  const currentInqId  = typeof defaultValues?.inquilino  === 'object' ? defaultValues.inquilino?.id  : defaultValues?.inquilino
  ...
  const occupiedInqIds = new Set(
    (contratosData?.results ?? [])
      ...
      .map(c => typeof c.inquilino === 'object' ? c.inquilino?.id : c.inquilino)
  )
  ...
  }, [habitacionId, allHabitaciones]) // eslint-disable-line react-hooks/exhaustive-deps

  const montoMensual = watch('monto_mensual')
  useEffect(() => {
    if (!dirtyFields.deposito) setValue('deposito', montoMensual || 0)
  }, [montoMensual]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <>
      <form onSubmit={handleSubmit((data) => onSubmit(data as unknown as ContratoWrite, cuotasGarantia))} className="flex flex-col gap-6">
