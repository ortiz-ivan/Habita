      ...
      <div className="rounded mb-8 bg-surface-1 border border-border">
        <div className="flex flex-wrap items-center gap-3 px-6 py-4">
          <div className="relative flex-1 min-w-[350px] max-w-xl">
            ...
            <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nombre del inquilino..." className={`${inpFilter} pr-3 w-full`} style={{ paddingLeft: '2.25rem' }} />
          </div>

          <SelectInput value={contratoId} onChange={(e) => setContratoId(e.target.value)} className={`${inpFilter} px-3`}>
          ...
          <SelectInput value={metodoPago} onChange={(e) => setMetodoPago(e.target.value)} className={`${inpFilter} px-3`}>
        ...
        <div className="flex items-center gap-1 flex-wrap px-6 py-3">
          ...
          {periodoPills.map((pill) => {
            ...
            return (
              <button key={pill.id} onClick={() => setPeriodo(pill.id)} className="flex items-center gap-1.5 text-[12px] px-3 py-[5px] rounded font-medium transition-colors cursor-pointer" style={isActive ? (pill.id === '' ? { backgroundColor: 'var(--color-brand)', color: '#FFFFFF' } : { backgroundColor: '#2a1200', color: 'var(--color-brand)' }) : { color: 'var(--color-stone-text)', backgroundColor: 'transparent' }} onMouseEnter={(e) => { if (!isActive) e.currentTarget.style.backgroundColor = 'var(--color-surface-2)' }} onMouseLeave={(e) => { if (!isActive) e.currentTarget.style.backgroundColor = 'transparent' }}>
                {pill.label}
              </button>
            )
          })}
