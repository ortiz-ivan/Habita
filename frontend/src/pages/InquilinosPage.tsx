...
export default function InquilinosPage() {
  ...
  const [editTarget, setEditTarget]     = useState<Inquilino | null>(null)
  const [viewTarget, setViewTarget]     = useState<Inquilino | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<Inquilino | null>(null)
  ...
  const openEdit   = (i: Inquilino) => { setEditTarget(i); setApiError(''); setViewTarget(null); setModalOpen(true) }

  const handleSubmit = (data: InquilinoWrite) => {
    setApiError('')
  ...
  ].filter(Boolean) as Array<{ key: string; isSearch?: true; label: string; onRemove: () => void }>
    ...
    <div>
      ...
      <div className="rounded mb-8 bg-surface-1 border border-border">
        <div className="flex flex-wrap items-center gap-3 px-6 py-4">
          ...
          {search && (
            <button onClick={() => setSearch('')} className="flex items-center gap-1.5 text-[13px] font-medium cursor-pointer transition-colors shrink-0" style={{ color: 'var(--color-stone-text)' }} onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--color-brand)' }} onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--color-stone-text)' }}>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          action={!search && <Button onClick={openCreate} className="px-5">+ Nuevo inquilino</Button>}
        />
      ) : (
        <>
      ...
      <Modal isOpen={!!viewTarget} onClose={() => setViewTarget(null)} title={`${viewTarget?.apellido}, ${viewTarget?.nombre}`} size="lg">
        {viewTarget && <InquilinoDetail i={viewTarget} onEdit={() => openEdit(viewTarget)} onDelete={() => setDeleteTarget(viewTarget)} />}
      </Modal>
